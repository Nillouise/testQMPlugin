...
namespace ima
{
	...
	{
	public:
		CBlock(int x = 0, int y = 0, int width = 0, int height = 0, const byte* pb = NULL) :CRectangle(x, y, width, height) 
		{
			if (pb == NULL)
				return; m_color[0] = *pb; m_color[1] = *(pb + 1); m_color[2] = *(pb + 2);
		}
		byte m_color[3];
		static int getBlock(const vector<byte*> &color, set<CBlock> &receive);
	};
